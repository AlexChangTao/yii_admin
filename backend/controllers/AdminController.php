...
class AdminController extends BaseController
{
    ...
    public function actionAdd(){
        ...
        $model = new Admin();
       // $ip = ip2long(Yii::$app->request->getUserIP()); exit($ip.'aa');//测试ip地址
        if (Yii::$app->request->isPost) {
            /* 表单验证 */
            $data = Yii::$app->request->post('Admin');
            $data['reg_time'] = time();
            $data['reg_ip']   = Yii::$app->request->getUserIP();
            $data['last_login_time'] = 0;
            $data['last_login_ip']   = Yii::$app->request->getUserIP();//ip2long('127.0.0.1');这个函数会把IP转化为计算后的整数有时容易出错
            $data['update_time']     = 0;
            ...
            //$e = $model->getAttribute('password');
            //exit('aa'.$e);
            /* 保存用户数据到数据库 */
            if ($model->save()) {
                $this->success('操作成功', $this->getForward());
            }else{
                $this->error('操作错误1', dump($model->errors) . '-' . $data['reg_ip']);
                //$this->error('操作错误1', $e);//后台可加任意数组
            }
        }
    ...
    public function actionEdit($uid){
        ...
        if (Yii::$app->request->isPost) {
            ...
            /* 保存用户数据到数据库 */
            if ($model->save()) {
                $this->success('操作成功', $this->getForward()); //实际是获取session中的Url值
            }else{
                $this->error('操作错误');
            }
        }

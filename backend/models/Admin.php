...
class Admin extends \common\models\Admin implements IdentityInterface
{
    ...
    {
        return static::findOne(['username' => $username, 'status' => self::STATUS_ACTIVE]); //static表示全局静态作用域，self::表示此类作用域
    }
    ...
    {
        return Yii::$app->security->validatePassword($password, $this->password); //自动加载数据库的值来比较
    }

...
<div class="portlet light bordered">
    ...
    <div class="portlet-body form">
        ...
        <?= $form->field($model, 'name')->iconTextInput([
       ...
       <div class="form-group">
            ...
            <div class="">
                <div class="input-icon left">
                    <i class="icon-lock"></i>
                    <input type="password" class="form-control c-md-2" name="Ulog[password]" placeholder="密码不变请留空" />
                </div>
            </div>
        </div>
        ...
        <!-- 单图 -->
        <?=$form->field($model, 'portrait_url')->widget('\common\widgets\images\Images',[
        ...
        <?=$form->field($model, 'account')->textInput(['class'=>'form-control c-md-1'])->label('当前积分')->hint('可用于积分消费')?>
        
        <?=$form->field($model, 'other')->textInput(['class'=>'form-control c-md-1'])->label('总积分')->hint('消费积分时，总积分不会变')?>
        
        <?= $form->field($model, 'ok_num')->radioList(['1'=>'正常','0'=>'隐藏'])->label('用户状态') ?>
